...
//  Exception e = new Exception("extractorsamplesource panpan test, in ExtractorSampleSource prepare");
//	e.printStackTrace();
    maybeStartLoading();
  ...
  public void seekToUs(long positionUs) {
    ...
    for (int i = 0; seekInsideBuffer && i < sampleQueues.size(); i++) {
	  Log.i("extractor", "extractor panpan test, in seekToUs, i = " + i);
      seekInsideBuffer &= sampleQueues.valueAt(i).skipToKeyframeBefore(positionUs);
    }

    // If we failed to seek within the sample queues, we need to restart.
    if (!seekInsideBuffer) {
	  Log.i("extractor", "extractor panpan test, in seekToUs, seekInsideBuffer no.");
      restartFrom(positionUs);
    }
	else
	{
	  Log.i("extractor", "extractor panpan test, in seekToUs, seekInsideBuffer yes.");
	}
  ...
  public TrackOutput track(int id) {
	...
	Exception e = new Exception("extractor exo panpan test");
	e.printStackTrace();
    return sampleQueue;
  }
  ...
  private ExtractingLoadable createLoadableFromStart() {
    Log.i("exoplayer", "exo panpan test, in createLoadableFromStart, go to new ExtractingLoadable.\n");
  ...
  private boolean haveFormatsForAllTracks() {
    Log.i("ExtractorSampleSource", "extractorsamplesource panpan test, in haveFormatsForAllTracks, sampleQueues.size = " + sampleQueues.size());
    for (int i = 0; i < sampleQueues.size(); i++) {
      ...
      while (result == Extractor.RESULT_CONTINUE && !loadCanceled) {
        ...
        try {
          ...
          while (result == Extractor.RESULT_CONTINUE && !loadCanceled) {
			...
			//Log.i("sample", "sample panpan test, in load, go to extractor.read.\n");
